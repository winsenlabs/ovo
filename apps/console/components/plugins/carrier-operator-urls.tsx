...
export function CarrierOperatorUrls({ bindingId }: { bindingId: string }) {
  ...
  useEffect(() => {
    ...
    void apiRequest<unknown>(`/provider-bindings/${encodeURIComponent(bindingId)}/carrier-urls`)
      .then(({ data }) => { if (active) { setUrls(carrierOperatorUrls(data)); setError(undefined); } })
      .catch(failure => { if (active) { setUrls([]); setError(failure instanceof Error ? failure.message : 'Carrier URLs unavailable'); } });
    return () => { active = false; };
  }, [bindingId]);
  return <section><h3>Operator URLs</h3><p>Paste these URLs into your carrier console.</p>
