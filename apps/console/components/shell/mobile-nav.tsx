...
export function MobileNav({ identity }: { identity: SessionIdentity }) {
  ...
  return <div className="mobile-nav">
    <button ref={trigger} type="button" className="button" aria-controls="mobile-navigation" aria-expanded={open} onClick={() => setOpen(value => !value)}>Menu</button>
    {open && <><button className="nav-scrim" aria-label="Close navigation" onClick={() => setOpen(false)} />
      <div id="mobile-navigation" ref={panel} className="mobile-nav-panel" role="dialog" aria-modal="true" aria-label="Navigation"><Sidebar identity={identity} onNavigate={() => setOpen(false)} /></div></>}
  </div>;
}
