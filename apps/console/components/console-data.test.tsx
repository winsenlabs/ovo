...
vi.mock('next/navigation', () => ({
  ...
  useRouter: () => ({ push: (url: string) => { nav.pushes.push(url); nav.search = url.split('?')[1] ?? ''; } }),
}));
...
class FakeEventSource {
  ...
  closed = false;
  constructor(url: string) { this.url = url; FakeEventSource.sources.push(this); }
  addEventListener(name: string, handler: EventListener) {
    this.listeners.set(name, [...(this.listeners.get(name) ?? []), handler as (event: MessageEvent) => void]);
  }
...
beforeEach(() => { nav.search = ''; nav.pushes = []; request.mockReset(); FakeEventSource.sources = []; });
afterEach(() => { cleanup(); vi.unstubAllGlobals(); vi.useRealTimers(); });

describe('console data surfaces', () => {
  function Pages() {
    const page = useCursorList<{ id: string }>('/agents?status=active', 50);
    return <><span data-testid="ids">{page.items.map(item => item.id).join(',')}</span>
      <span data-testid="state">{page.status}</span>
      <button disabled={!page.hasNext} onClick={page.next}>Next page</button>
      <button disabled={!page.hasPrevious} onClick={page.previous}>Previous page</button></>;
  }

  it('fetches cursor pages and keeps next and previous in the URL', async () => {
    request.mockImplementation(async (path: string) => ({ data: path.includes('cursor=page-2')
      ? { items: [{ id: 'second' }] } : { items: [{ id: 'first' }], nextCursor: 'page-2' } }));
  ...
  it('deduplicates the replay cursor and reconnects from the last event', () => {
    ...
    act(() => { first.onopen?.(); first.emit('transcript', { text: 'hello' }, '7'); first.emit('transcript', { text: 'hello' }, '7'); });
  ...
  it('uses heartbeat liveness and closes the stream on unmount', () => {
    ...
    act(() => { source.onopen?.(); vi.advanceTimersByTime(3000); });
    expect(screen.getByTestId('stream-status').textContent).toBe('stale');
  ...
  it('renders table headers as mobile card labels and an honest empty state', () => {
    ...
    const view = render(<DataTable label="Agents" rows={[{ id: 'a1' }]} columns={columns} rowKey={row => row.id} />);
    ...
    view.rerender(<DataTable label="Agents" rows={[]} columns={columns} rowKey={row => row.id} empty="No agents returned" />);
  ...
  it('describes API errors without leaking an object string', () => {
    ...
    expect(describeError({ code: 'bad_request', message: 'Invalid field' })).not.toContain('[object Object]');
  });
});
