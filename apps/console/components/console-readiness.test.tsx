...
describe('release readiness detail', () => {
  it('renders slot and stage blockers with a link to the corrective plugin control', () => {
    render(
      <StudioRail
        ...
        readiness={{
          releaseReady: false,
          requiredPluginIds: [],
          blockers: ['Carrier format is incompatible'],
          details: [
            {
    ...
    expect(screen.getByRole('link', { name: 'Pick another carrier' }).getAttribute('href')).toBe(
      '/agents/agent-1/plugins#slot-carrier',
    );
  });
