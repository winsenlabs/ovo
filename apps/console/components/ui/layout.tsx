...
export function Cluster({ children, className = '' }: Props) {
  return <div className={`ui-cluster ${className}`}>{children}</div>;
}
export function Grid({ children, className = '' }: Props) {
  return <div className={`ui-grid ${className}`}>{children}</div>;
}
...
export function Toolbar({ children }: Props) {
  return (
    <div className="ui-toolbar" role="toolbar">
      {children}
    </div>
  );
}
