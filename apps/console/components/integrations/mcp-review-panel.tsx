...
import type { AgentDraft, DiscoveredTool, McpConnection, SessionIdentity, ToolApproval } from '../../lib/api';
import { EmptyState, Field, JsonEvidence, Notice, Panel, PanelHeader, StatusBadge } from '../primitives';
export function McpReviewPanel({ drifted, connection, connections, setSelectedConnection, agent, agents, setSelectedAgent, role, connectionAction, discovered, approvals, approve }: {
  drifted: DiscoveredTool[]; connection?: McpConnection; connections: McpConnection[];
  setSelectedConnection: Dispatch<SetStateAction<string | undefined>>;
  agent?: AgentDraft; agents: AgentDraft[]; setSelectedAgent: Dispatch<SetStateAction<string | undefined>>;
  role: SessionIdentity['role']; connectionAction: (action: 'test' | 'discover') => Promise<void>;
  discovered: DiscoveredTool[]; approvals: ToolApproval[]; approve: (tool: DiscoveredTool) => Promise<void>;
}) {
  return (
