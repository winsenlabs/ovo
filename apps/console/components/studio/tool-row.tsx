...
export function ToolRow({ tool, index, config, update, patch, onRemove, emptyTool }: {
  tool: Tool; index: number; config: AgentConfig;
  ...
  return (
    <fieldset className="nested-card">
            ...
            {tool.connector === 'http' && tool.http && <HttpToolFields tool={tool as Tool & { http: NonNullable<Tool['http']> }} index={index} patch={patch} />}
            {tool.connector === 'mcp' && (
              <div className="form-grid">
