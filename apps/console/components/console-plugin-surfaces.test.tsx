...
describe('manifest-driven console controls', () => {
  ...
  it('does not offer a secret field in binding configuration', () => {
    const secretPlugin = { ...plugin, secretFields: ['/token'], bindingSchema: { properties: { token: { type: 'object' } }, required: ['token'] } };
    render(<BindingSelect plugin={secretPlugin} bindings={[]} credentials={[{ id: 'credential-1', label: 'Stored API key', provider: 'fixture', type: 'api-key', environment: 'production' }]} onChange={vi.fn()} />);
    expect(screen.queryByLabelText('token')).toBeNull();
    expect(screen.getByText(/Use the binding credential above/)).toBeDefined();
    expect((screen.getByLabelText('Credential').closest('form')!.querySelector('button[type=submit]') as HTMLButtonElement).disabled).toBe(true);
    expect(request).not.toHaveBeenCalled();
  });

  it('saves a write-only credential before creating a binding and keeps its value out of config', async () => {
    request.mockImplementation(async (path: string) => path === '/credentials'
      ? { data: { id: 'credential-new', label: 'Fixture carrier credential', provider: 'fixture', type: 'carrier', environment: 'production', fingerprint: 'fp-123' } }
      : { data: { id: 'binding-new', label: 'New carrier', provider: 'fixture', pluginId: plugin.id, credentialId: 'credential-new', environment: 'production' } });
    render(<BindingSelect plugin={plugin} bindings={[]} credentials={[]} onChange={vi.fn()} />);
    ...
    await waitFor(() => expect(request).toHaveBeenCalledWith('/credentials', expect.objectContaining({
      method: 'POST', body: expect.stringContaining('"value":"private-key"'),
    })));
    ...
    await waitFor(() => expect(request).toHaveBeenCalledWith('/provider-bindings', expect.objectContaining({
      method: 'POST', body: expect.stringContaining('"credentialId":"credential-new","config":{}'),
    })));
  });

  it('uses a credential reference for an agent config secret pointer', () => {
    const onChange = vi.fn();
    render(<SchemaForm plugin={{ ...plugin, secretFields: ['/apiKey'] }} schema={{ properties: { apiKey: { type: 'object' } }, required: ['apiKey'] }}
      value={{}} onChange={onChange} credentials={[{ id: 'credential-1', label: 'Stored API key', provider: 'fixture', type: 'carrier', environment: 'production' }]} />);
    fireEvent.change(screen.getByLabelText('apiKey'), { target: { value: 'credential-1' } });
    expect(onChange).toHaveBeenCalledWith({ apiKey: { credentialRef: { credentialId: 'credential-1' } } });
  ...
  it('makes a newly saved binding credential available to the sibling config selector', async () => {
    const secretPlugin = { ...plugin, secretFields: ['/apiKey'], configSchema: { properties: { apiKey: { type: 'object' } } } };
    request.mockResolvedValue({ data: { id: 'credential-new', label: 'Fixture carrier credential', provider: 'fixture', type: 'carrier', environment: 'production', fingerprint: 'fp-123' } });
    function PluginControls() {
      const [credentials, setCredentials] = useState<{ id: string; label: string; provider: string; type: string; environment: string; fingerprint?: string }[]>([]);
      const [config, setConfig] = useState<Record<string, unknown>>({});
      return <><BindingSelect plugin={secretPlugin} bindings={[]} credentials={credentials} onChange={vi.fn()}
        onCredentialCreated={credential => setCredentials(current => [...current, credential])} />
        <SchemaForm plugin={secretPlugin} schema={secretPlugin.configSchema} value={config} onChange={setConfig} credentials={credentials} />
    ...
    fireEvent.change(screen.getByLabelText('New credential value'), { target: { value: 'private-key' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save credential', hidden: true }));
    ...
    expect(screen.getByTestId('agent-config').textContent).toBe('{"apiKey":{"credentialRef":{"credentialId":"credential-new"}}}');
  });

  it('clears the draft credential and config when a slot switches plugins', async () => {
    request.mockResolvedValue({ data: { id: 'credential-new', label: 'First credential', provider: 'fixture', type: 'carrier', environment: 'production', fingerprint: 'fp-123' } });
    const view = render(<BindingSelect plugin={{ ...plugin, bindingSchema: { properties: { region: { type: 'string' } } } }} bindings={[]} credentials={[]} onChange={vi.fn()} />);
    fireEvent.change(screen.getByLabelText('region'), { target: { value: 'region-A' } });
    fireEvent.change(screen.getByLabelText('New credential value'), { target: { value: 'first-secret' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save credential', hidden: true }));
    await waitFor(() => expect((screen.getByLabelText('Credential') as HTMLSelectElement).value).toBe('credential-new'));
    expect(screen.getByText('Stored · fingerprint fp-123')).toBeDefined();
    request.mockClear();

    view.rerender(<BindingSelect plugin={{ ...plugin, id: 'other-carrier', provider: 'other', bindingSchema: { properties: { region: { type: 'string' } } } }} bindings={[]} credentials={[]} onChange={vi.fn()} />);
