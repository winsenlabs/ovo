...
export function ProvidersFeature() {
  ...
  return <Integrations identity={identity} initialTab="providers" />;
}
export function ToolsFeature() {
  const identity = useSession();
  return <Integrations identity={identity} initialTab="tools" />;
}
export function CampaignsFeature() {
  const identity = useSession();
  return <CampaignsView role={identity.role} />;
}
export function SuppressionsFeature() {
  const identity = useSession();
  return <SuppressionsView role={identity.role} />;
}
export function HandoffsFeature() {
  const identity = useSession();
  return <HandoffsView role={identity.role} />;
}
export function InboundFeature() {
  const identity = useSession();
  return <InboundRoutes role={identity.role} />;
}
export function EvaluationsFeature() {
  const identity = useSession();
  return <EvaluationDatasetsView role={identity.role} />;
}
export function CostsFeature() {
  const identity = useSession();
  return <CostView role={identity.role} />;
}
export function PerformanceFeature() {
  return <PerformanceView />;
}
export function InfrastructureFeature() {
  return <InfrastructureView />;
}
export function TeamFeature() {
  const identity = useSession();
  return <TeamView identity={identity} onSessionRevoked={() => location.assign('/login')} />;
}
export function AccountFeature() {
  const identity = useSession();
  return <AccountView identity={identity} onSessionRevoked={() => location.assign('/login')} />;
}
