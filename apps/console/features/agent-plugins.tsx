...
export function AgentPluginsFeature({ agentId }: { agentId: string }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setDraft(normalizeDraft(agent.data, agent.etag)); setCatalog(plugins.data);
      setBindings(items(bindingRows.data)); setCredentials(items(credentialRows.data));
    } catch (failure) { setError(failure instanceof Error ? failure.message : 'Agent plugins unavailable'); }
  }, [agentId]);
  ...
  useEffect(() => {
    if (!draft) return;
    let active = true;
    void apiRequest<CompatIssue[]>('/plugins/compat', { method: 'POST', body: JSON.stringify({ voice, mode: draft.config.mode, language: draft.config.language, tools: draft.config.tools }) })
      .then(({ data }) => { if (active) setIssues(data); })
      .catch(failure => { if (active) setError(failure instanceof Error ? failure.message : 'Compatibility unavailable'); });
    return () => { active = false; };
  ...
  async function save() {
    ...
    try {
      const result = await apiRequest<unknown>(`/agents/${encodeURIComponent(agentId)}`, { method: 'PUT', headers: { 'if-match': ifMatch(draft.draftVersion) }, body: JSON.stringify({ config: draft.config }) });
      setDraft(normalizeDraft(result.data, result.etag));
    } catch (failure) { setError(failure instanceof Error ? failure.message : 'Plugin choices could not be saved'); }
    finally { setSaving(false); }
  }
  if (!draft) return <p role="status">{error ?? 'Loading plugin choices…'}</p>;
  return <div className="ui-stack"><PageHeader eyebrow="Agent plugins" title="Choose voice plugins" description="Selections are checked against installed manifests and binding metadata." actions={<button className="button primary" type="button" disabled={saving || identity.role === 'viewer'} onClick={() => void save()}>{saving ? 'Saving…' : 'Save plugins'}</button>} />
    {error && <Callout tone="danger">{error}</Callout>}
    {slots.filter(slot => slot !== 'llm' || (draft.config.mode !== 'announcement' && draft.config.mode !== 'faq')).map(slot => {
