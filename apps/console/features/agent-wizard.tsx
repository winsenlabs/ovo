...
export function AgentWizardFeature() {
  ...
  return <div className="ui-stack"><PageHeader eyebrow="Create agent" title="New voice agent" description="Name the agent, choose installed plugins, then add the behavior essentials." />
    ...
    {step === 1 && <div className="ui-stack"><p>No carrier is preselected. Choose the one configured for your number when ready.</p>
      {wizardSlots.filter(slot => slot !== 'llm' || (config.mode !== 'announcement' && config.mode !== 'faq')).map(slot => <SlotPicker key={slot} slot={slot} plugins={catalog.plugins} value={voice[slot]?.plugin} voice={voice} mode={config.mode} language={config.language}
        onChange={plugin => setConfig(current => ({ ...current, voice: { ...(current.voice ?? { textFilters: [], acknowledgements: [] }), [slot]: { plugin, config: {} } } }))} />)}
    </div>}
    {step === 2 && <section className="panel panel-body ui-stack"><label>{config.mode === 'announcement' ? 'Approved message' : 'Opening message'}<textarea value={config.message} onChange={event => setConfig(current => ({ ...current, message: event.target.value }))} /></label>
      {(config.mode === 'context' || config.mode === 'agent') && <label>Grounding context<textarea value={config.context} onChange={event => setConfig(current => ({ ...current, context: event.target.value }))} /></label>}
