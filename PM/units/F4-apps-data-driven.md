...
- packages/plugin-operations/src/inbound-overflow.ts (unchanged overflow-policy extraction for the size gate while recording carrier-configuration refusals)
...
- packages/plugin-voice/src/scheduler.ts and packages/plugin-media/tests/gateway.integration.test.ts (first-turn clear correction and real gateway regression; E2/C2)
- packages/plugin-media/src/{gateway,worker-client,worker-media-session}.ts and tests/{gateway-socket-fixture,worker-client.integration.test}.ts (bounded accept gate and pre-accept per-session close; C2)
- packages/plugin-kit/src/duplex-shims.ts and tests/speech-shims.test.ts (explicit legacy end-reason mapping; F2/O1)
- packages/session-host/src/terminate.ts (typed carrier media termination reason; O1/C2)
- packages/plugin-operations/src/index.ts (export of the explicit unarmed-gate error; O2/C2)
...
**Checker note (2026-09-24, second re-check).** The checker reproduced two blockers at the first real call: the native scheduler cleared media before `session.accept`, and worker telemetry converted a v2 STT session into a v1-shaped object without `cancel`, preventing input-enabled sessions from closing. F4 repaired both because the gateway and lifecycle could not complete otherwise. `plugin-voice/src/scheduler.ts` is a shared E2 touchpoint; `plugin-media/tests/gateway.integration.test.ts` is a shared C2 harness touchpoint. The worker telemetry seam and new input-enabled lifecycle test belong to D1/O1. The same re-check found four F4 defects: the distribution-default engine escaped release dependency validation, termination erased typed reasons, NULL inbound carrier selection failed despite one installed control, and admission could proceed with an unarmed carrier gate. The corresponding API, worker, gateway and operations edits stay within those obligations. I1 inherits the selected-slot validation and test fixture; C2 inherits the gateway fallback and explicit unarmed refusal; O2 inherits the admission assertion. The admission change checks only that the local gate was installed, before SQL; ownership, capacity, idempotency and transaction boundaries are unchanged.

**Checker note (2026-09-24, termination signature).** The prior F4 and C2 text called `terminate(sessionId)` and had C2 send the literal reason `terminate`; that would erase the typed reason before a close-stream carrier disposed the engine. The second re-check changed the seam to `terminate(sessionId, reason: EndReason)` and updated design §4.10 and the future C2 spec accordingly. C2 must forward the actual reason on `session.end`.

**Checker note (2026-09-24, round 3).** The first-call gateway ordering previously depended on four microtask hops of slack. F4 now holds bounded media frames in `WorkerMediaSession` until its `session.accept` frame has been sent, and the gateway treats a pre-accept `session.close` as a refusal of that session only. A test runs the real gateway, loaded worker distribution, production factory and media runtime with four added awaits and a 20 ms async scheduling delay; its broken-gate mutation fails. `plugin-media` sources and tests are shared C2 touchpoints; E2 inherits the production first-call regression. API release validation recognizes the same host session capabilities provided by the worker, and an API/worker test composes `context` and `agent` with the actual first-party OpenAI bridge; I1 inherits it. The carrier-configuration selection now happens before the admission write transaction, which records a durable refusal and returns 503 at the webhook; wait resumes also persist a refusal if their snapshotted carrier is no longer installed. `inbound-overflow.ts` is an unchanged helper extraction under the module-size gate. O2 inherits these operations edits, and C2 inherits startup arming and the constrained env fallback. The lease-loss/shutdown finalization gap remains an O1/O2 follow-up recorded on the board.
...
| D1    | `apps/api/src/release-simulation.ts`; `apps/worker/src/{legacy-session-compat,recording-evidence,session-graph-runtime,session-graph-host,speech-cache-v2,v1-engine-adapter}.ts`; `apps/worker/tests/{f4-recording-evidence,f4-session-graph,v1-engine-adapter,weak-playback,production-session-lifecycle,input-enabled-session-lifecycle}.test.ts` |
...
| I1    | `apps/api/src/{release-catalog,release-graph,release-selections}.ts`, `apps/api/src/routes/{plugins,registry}.ts`, `apps/api/tests/{f4-release-validation,f4-routes,f4-selection-release,f4-live-readiness,f4-companion-selection,selected-speech-fixture}.ts`                                                                                      |
...
- worker-media-bootstrap.ts: give createProductionWorkerMediaRuntime its FINAL signature. It takes {httpServer: http.Server (the health server from worker-health.ts on PORT, default 4100), ...} and returns {start(), close(reason), closeSession(sessionId, reason), terminate(sessionId, reason: EndReason)}. Internally it still uses the legacy WorkerGatewayClient in this wave; start() calls connect(). C2 later swaps the internals without changing the signature. main.ts must pass the health server.
...
- Termination: behavior completion, ownership loss, drain and shutdown go through the session-host terminateCarrierLeg (section 4.10). A mediaRuntime.terminate(sessionId, reason) that can't run yet in the legacy topology falls back to closing the carrier session with that typed reason. Keep the regression test that ownership loss hangs up.
