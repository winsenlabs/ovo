...
| [F4](F4-apps-data-driven.md)  | API and worker made data-driven                                                         | F1–F3      | 1, 20, 21, 26      | Built - awaiting check                                                                                                                                                                                                                                                          |
...
| F3   | The call-input predicate still has copies in session-host, worker and API live readiness. F4 must use the session-host predicate for both application paths; the duplication gate does not detect these short copies.                                                                                                                                                       | F4         |
...
| F4   | C2 can read the raw route carrier selection from the inbound admission's job payload and durable rows; it must compose the installed control for that selection. Its separate obligation to pass the actual route carrier as `CompatInput.actualCarrier` before admission remains open.                                                                                     | C2         |
