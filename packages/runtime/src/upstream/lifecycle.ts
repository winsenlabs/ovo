/** Adapted from DeepSeek Harness ddefc45fbc7f8e46dd73185e68295696d1297887; MIT, see LICENSE. */
/**
...
// OVO loads only operator-approved code, never sandbox/model-installed plugins.
 ...
 * @param plugin - the plugin the sandbox returned; operator-approved plugin, validated before mounting.
 * @param config - immutable release configuration.
...
export async function startHostHalf(
  ...
  await group.await()
  const fiber = group.ctx.plugin(plugin, config)
  try {
    await fiber.await()
  } catch (error) {
    await fiber.dispose()
