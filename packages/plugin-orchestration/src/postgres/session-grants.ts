...
function checkScope(input: { organizationId: string; carrierId: string }): void {
  if (!input.organizationId || !input.carrierId)
...
async function lockRoute(
  ...
  if (carrierCallId)
    await lockCarrierCallId(
      client,
      preview.rows[0]!.organization_id,
      preview.rows[0]!.carrier_id,
      carrierCallId,
    );
    ...
    return transaction(this.pool, async (client) => {
      const row = await lockRoute(
        ...
        `($1::uuid IS NULL OR session_id = $1) AND ($2::text IS NULL OR dial_request_id = $2)
         AND organization_id = $3 AND carrier_id = $4`,
        [
      ...
      if (column === 'carrier_call_id' && row.carrier_stream_call_id)
    ...
    return transaction(this.pool, async (client) => {
      ...
      const row = await lockRoute(
        ...
        `($1::text IS NULL OR dial_request_id = $1)
           ...
           AND organization_id = $4 AND carrier_id = $5`,
        [
          input.dialRequestId ?? null,
      ...
      if (
        ...
        return undefined;
      if (
        input.streamCallIdMatchesDial === true &&
        input.carrierCallId &&
        row.carrier_call_id &&
        input.carrierCallId !== row.carrier_call_id
      )
        return undefined;
      const ownership = await client.query(
        `SELECT 1 FROM ovo_jobs WHERE id = $1 AND owner_id = $2 AND owner_epoch = $3
           AND status IN ('dialing', 'accepted') AND lease_expires_at > now()`,
        [row.job_id, row.worker_id, row.owner_epoch],
      );
      if (ownership.rowCount !== 1) return undefined;
      const slot = await client.query(
        `SELECT 1 FROM ovo_worker_slots WHERE worker_id = $1 AND ownership_epoch = $2
           AND state IN ('reserved', 'active') AND lease_expires_at > now()`,
        [row.worker_id, row.worker_slot_epoch],
      );
      if (slot.rowCount !== 1) return undefined;
      ...
      if (input.carrierCallId && input.carrierCallId !== primary && input.carrierCallId !== alias) {
        if (
          ...
          return undefined;
        if (!primary && !alias) {
          if (input.streamCallIdMatchesDial === true) primary = input.carrierCallId;
          else alias = input.carrierCallId;
        } else if (primary && !alias) alias = input.carrierCallId;
        ...
        `UPDATE ovo_session_routes SET carrier_call_id = $2, carrier_stream_call_id = $3,
         ...
         WHERE session_id = $1 AND ${grantable}
           AND EXISTS (SELECT 1 FROM ovo_jobs j WHERE j.id = ovo_session_routes.job_id
             ...
             AND j.status IN ('dialing', 'accepted') AND j.lease_expires_at > now())
    ...
    const updated = await this.pool.query<SessionRouteRow>(
      `UPDATE ovo_session_routes r SET generation = generation + 1,
       ...
       WHERE (r.carrier_call_id = $1 OR r.carrier_stream_call_id = $1)
         AND r.organization_id = $5 AND r.carrier_id = $6
         ...
         AND NOT EXISTS (SELECT 1 FROM ovo_session_routes x WHERE x.session_id <> r.session_id
           AND x.organization_id = $5 AND x.carrier_id = $6
