...
- `@winsendotai/ovo-behaviors`
  ...
  - Agent behavior: bounded by `maxSteps`, exact allowlist and Ajv tool-input validation, local error records for rejected tool selections, and business execution only through the shared `Execution` interface.
- `@winsendotai/ovo-plugin-inference`
  - Vercel AI SDK `generateText` adapter pinned to `ai@7.0.107`, with `maxRetries: 0`, one provider step, abort propagation, declared JSON Schema tools without SDK `execute` handlers, and normalized usage evidence.
...
The focused suite covers generated versus completed evidence, stale-epoch cancellation, queue overflow, a provider that never settles, bounded evidence retention, safe formatted announcements, schema failure, unsafe template rejection, FAQ ambiguity/tool/negation conservatism, context budget failure before inference, bounded agent tool continuation through `Execution`, rejection plus recording of unknown tools, all four modes composed through the shared runtime, one AI SDK provider call without tool execution, and simulated inference abort.

Observed: 3 test files passed, 16 tests passed.

Owned-package TypeScript validation uses a temporary project containing only these three package source trees and reports no errors. The architecture, namespace, private-package, PM-criteria, and pinned-upstream portions of `pnpm lint` pass. The combined lint command remains red only because its final repository-wide module-size stage reports concurrent out-of-scope modules.

All owned production TypeScript modules are below 300 canonical nonblank lines and 24 KiB. The largest is the scheduler at roughly 205 canonical nonblank lines; AI SDK, simulated inference, and plugin/config wiring are separate modules. The repository-wide size gate still reports concurrent out-of-scope modules owned by other work streams; no owned voice/behavior/inference file appears in that failure list.
